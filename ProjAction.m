...
///
// Gives the action of matrix M on the coinduced module C[P^1(Z_K/J)]
//
/*
ProjMat:=function(K,J,M)
     ...
     FacMatrix:=[**];
      ...
      Append(~FacMatrix,PermutationMatrix(K,perm));
...
ProjMat2:=function(K,J,M)
    ...
    for f in fac do
      PL,r:=ProjectiveLine(quo<O|f[1]^f[2]>);
      level:=level*(#PL);
     ...
     Insert(~FacMatrix,1,level);

     return FacMatrix;
end function;


ProjMatOld:=function(K,J,M)
